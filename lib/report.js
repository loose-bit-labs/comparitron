...
function run(runId) {
  ...
  const lbTpsFor = (model) => lbRows.find(r => r.model === model)?.tps ?? null
  ...
  // rankings
  lines.push('---', '', '## Rankings', '')
  const rankHeaders = ['Model', 'hw', 'Score', 'gen t/s', 'prefill t/s', ...config.capabilities.map(s => s.slice(0, 9)), 'Votes', 'Self Δ']
  ...
  lines.push(`> **Score** = scenario-weighted average of per-prompt jury scores (max ${data.maxScore}). **gen t/s** = generation speed (eval phase). **prefill t/s** = prompt ingestion speed. Both from a single run — treat as directional, not precise.  `)
  ...
  for (const r of data.rows) {
    ...
    const entry = {
      ...
      hardware: hw,
      lastTested: date,
      tps: lbRows.find(lb => lb.model === r.model && lb.hardware === hw)?.tps ?? r.avgSpeed ?? null,
