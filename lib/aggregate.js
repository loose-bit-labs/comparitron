...
const DEFAULT_WEIGHTS = { instruction_following: 2, correctness: 4, conciseness: 1, format_compliance: 2 }
// Legacy exports — use config.weights at runtime; these are fallbacks for callers that don't pass config
const WEIGHTS = DEFAULT_WEIGHTS
const MAX_SCORE = Object.values(DEFAULT_WEIGHTS).reduce((a, b) => a + b, 0) * 5
...
function weightedScore(scoreObjs, weights = DEFAULT_WEIGHTS) {
...
function aggregate(config, runId = null) {
  ...
  const peerObjs = {}   // model → [{ score, weight }]
  const selfObjs = {}   // model → [{ score, weight }]
  ...
  for (const [key, objs] of Object.entries(byPromptPeer)) {
    ...
    const s = weightedScore(objs, weights)
    const w = scenarioWeights[scenario] ?? 1
    if (s != null) {
      ;(peerObjs[candidate] ??= []).push({ score: s, weight: w })
  ...
  for (const [key, objs] of Object.entries(byPromptSelf)) {
    ...
    const s = weightedScore(objs, weights)
    const w = scenarioWeights[scenario] ?? 1
    if (s != null) (selfObjs[candidate] ??= []).push({ score: s, weight: w })
  }
  ...
  const rows = models.map(model => {
    ...
    const peer = wavg(peerObjs[model] || [])
    const self = wavg(selfObjs[model] || [])
    const delta = peer != null && self != null ? self - peer : null
    const promptsScored = (peerObjs[model] || []).length
    const votes = allScores.filter(s => s.candidate === model && s.candidate !== s.juror).length
    return { model, avgSpeed, avgPrefill, sceneCols, sceneVotes, sceneDims, peer, self, delta, votes, promptsScored }
